...
async def rules(call: CallbackQuery, bot: Bot, state: FSMContext):
    try:
        language = call.data.split('_')[1]
        if language == 'ru':
            conn = sqlite3.connect('shop.db')
            cursor = conn.cursor()
            cursor.execute('''SELECT text_rules, photo_rules FROM oform_ru WHERE id = ?''', (1,))
            ...
            conn = sqlite3.connect('shop.db')
            cursor = conn.cursor()
            cursor.execute('''SELECT text_rules, photo_rules FROM oform_en WHERE id = ?''', (1,))
